...
describe('Merge', function () {
    ...
    it("Comprobando que se ha integrado la rama 'sine' en la rama master", async function () {
        this.score = 3;
        if (error_critical) {
            this.msg_err = error_critical;
            should.not.exist(error_critical);
            ...
            this.msg_ok = `La rama 'sine' se ha integrado en master`;
            this.msg_err = `El último commit ${output} de la rama 'sine' no se ha integrado con el commit ${expected} de la rama master`;
            commit_head_sine = output;
            Utils.search(expected, output).should.be.equal(true);
        }
    });

    it("Buscando 'sin' en el contenido integrado en la rama master", async function () {
        const expected = "sin";
        this.score = 3;
        if (error_critical) {
            this.msg_err = error_critical;
            ...
            let output;
            this.msg_ok = `Se ha encontrado '${expected}' en la rama master ${commit_head_sine}`;
            this.msg_err = `No se ha encontrado '${expected}' en la rama master ${commit_head_sine}`;
